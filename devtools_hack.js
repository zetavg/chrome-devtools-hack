if (location.protocol === 'chrome-devtools:') {
  ...
  function __hack() {
    ...
    // Check if devtools is fully initialized
    if (document.getElementsByClassName('split-view-contents')[0] && document.getElementsByClassName('sidebar-pane-toolbar')[0] && document.getElementsByClassName('matched-styles')[0] && !__hack_init) {
      ...
      document.getElementsByClassName('sidebar-pane-toolbar')[0].insertBefore(searchBox, document.getElementsByClassName('sidebar-pane-toolbar')[0].childNodes[99]);


      var __hack_cssSearch = function () {
        var s = document.getElementsByClassName('sidebar-pane-toolbar')[0].getElementsByClassName('css-search-box')[0].value;
        var styleType = ['navigable', 'read-only'];
        for (t=0; t<styleType.length; t++) {
          var styles = document.getElementsByClassName('styles-pane')[0].getElementsByClassName(styleType[t]);
      ...
      document.getElementsByClassName('sidebar-pane-toolbar')[0].getElementsByClassName('css-search-box')[0].addEventListener('change', __hack_cssSearch, false);
      document.getElementsByClassName('sidebar-pane-toolbar')[0].getElementsByClassName('css-search-box')[0].addEventListener('keyup', __hack_cssSearch, false);
      document.getElementsByClassName('sidebar-pane-toolbar')[0].getElementsByClassName('css-search-box')[0].addEventListener('paste', __hack_cssSearch, false);
      ...
      console.log('resting.')
      setTimeout(__hack, 1200);
    }
  }

  setTimeout(__hack, 2400);
  setTimeout(__hack, 3600);
  setTimeout(__hack, 5200);
  setTimeout(__hack, 9900);
}
